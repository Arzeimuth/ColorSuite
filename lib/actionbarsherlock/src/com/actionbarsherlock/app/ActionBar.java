 ...
 * {@link android.R.style#Theme_Holo Holo} theme (or one of its descendant themes), which is the default. You may otherwise add the action bar by calling
 * {@link android.view.Window#requestFeature requestFeature(FEATURE_ACTION_BAR)} or by declaring it in a custom theme with the
 * {@link android.R.styleable#Theme_windowActionBar windowActionBar} property.
 * <p>
 * By default, the action bar shows the application icon on the left, followed by the activity title. If your activity has an options menu, you can make select
 * items accessible directly from the action bar as "action items". You can also modify various characteristics of the action bar or remove it completely.
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * In some cases, the action bar may be overlayed by another bar that enables contextual actions, using an {@link android.view.ActionMode}. For example, when
 * the user selects one or more items in your activity, you can enable an action mode that offers actions specific to the selected items, with a UI that
 * temporarily replaces the action bar. Although the UI may occupy the same space, the {@link android.view.ActionMode} APIs are distinct and independent from
 * those for {@link ActionBar}. <div class="special reference">
 * <h3>Developer Guides</h3>
 * <p>
 * For information about how to use the action bar, including how to add action items, navigation modes and more, read the <a href="{@docRoot}
...
public abstract class ActionBar
{
  ...
  /**
   * Set the action bar's subtitle. This will only be displayed if {@link #DISPLAY_SHOW_TITLE} is set. Set to null to disable the
   * subtitle entirely.
   * 
   * @param subtitle
   *          Subtitle to set
   ...
   * 
   * <p>
   * Example: setDisplayOptions(0, DISPLAY_SHOW_HOME) will disable the {@link #DISPLAY_SHOW_HOME} option. setDisplayOptions(DISPLAY_SHOW_HOME, DISPLAY_SHOW_HOME
   * | DISPLAY_USE_LOGO) will enable {@link #DISPLAY_SHOW_HOME} and disable {@link #DISPLAY_USE_LOGO}.
   * 
   * @param options
   *          A combination of the bits defined by the DISPLAY_ constants
   *          defined in ActionBar.
   ...
   * Returns null if {@link #getNavigationMode()} would not return {@link #NAVIGATION_MODE_STANDARD}.
   * 
  ...
  /**
   * Add a tab for use in tabbed navigation mode. The tab will be inserted at <code>position</code>. If this is the first tab to be added it will become
   * the selected tab.
   * 
   * @param tab
   *          The tab to add
   ...
   * If the window hosting the ActionBar does not have the feature {@link Window#FEATURE_ACTION_BAR_OVERLAY} it will resize application
   ...
   * 
   * <p>
   * This defaults to true for packages targeting &lt; API 14. For packages targeting API 14 or greater, the application should call this method to enable
  ...
  public static abstract class Tab
  {
    ...
    /**
     * Set a custom view to be used for this tab. This overrides values set by {@link #setText(CharSequence)} and {@link #setIcon(Drawable)}.
     * 
     * @param view
